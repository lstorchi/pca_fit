...
int main (int argc, char ** argv)
{
  ...
  readingfromfile (filename, ptin, phiin, d0in, etain, z0in, coordin, 
  ...
  if (selectsubladder || selectsubsecor)
  {
    if (selectsubsecor)
    {
      ...
      select_bigger_sub (subsectors, verbose, maxnumber, slctsubsec);
      
      std::cout << "Selected subsector " << slctsubsec << " numevt: " << maxnumber << std::endl;

      extract_sub (subsectorslist, slctsubsec, ptin, phiin,
    ...
    if (selectsubladder)
    {
      ...
      select_bigger_sub (subladders, verbose, maxnumber, slctsubsec);
      
      std::cout << "Selected subladder " << slctsubsec << " numevt: " << maxnumber << std::endl;

      extract_sub (subladderslist, slctsubsec, ptin, phiin,
          d0in, etain, z0in, coordin, pt, 
          phi, d0, eta, z0, coord);
    }
  ...
  compute_pca_constants (pt, phi, d0, eta, z0, coord, cmtx, q);
