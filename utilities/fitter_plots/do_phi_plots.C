...
void do_phi_plots(unsigned int flag=0){

  TString filename = "phi_fitted_results.txt";
  ...
  if ( flag == 1 ) {
    ...
    for (int itow=0; itow<16; ++itow) {
      g_phi_bias[itow] = new TGraphErrors(7,pt_bin,phi_bias[0][itow],pt_bin_width,phi_bias_e[0][itow]);
	...
	g_phi_bias[itow]->SetMinimum(-18.e-6);
	g_phi_bias[itow]->SetMaximum( 18.e-6);
	g_phi_bias[itow]->GetXaxis()->SetTitle("p_{T}^{bin} [GeV/c]");
	g_phi_bias[itow]->GetYaxis()->SetTitle("#LT#phi_{gen}-#phi_{fit}#GT [rad]");
    ...
    for (int itow=0; itow<16; ++itow) {
      g_phi_sigma[itow] = new TGraphErrors(7,pt_bin,phi_sigma[0][itow],pt_bin_width,phi_sigma_e[0][itow]);
	...
	g_phi_sigma[itow]->SetMinimum(0.);
	g_phi_sigma[itow]->SetMaximum(0.0020);
	g_phi_sigma[itow]->GetXaxis()->SetTitle("p_{T}^{bin} [GeV/c]");
	g_phi_sigma[itow]->GetYaxis()->SetTitle("#sigma_{#phi_{gen}-#phi_{fit}} [rad]");
  ...
  //====================================================================================================================
    ...
    //  cout << " ----> " << itow << endl;
    //  for ( int i=0; i<7; ++i){
    //	cout << "    bin " << i << "  -  " <<  phi_bias_m5[0][itow][i] << " " <<  phi_bias_m6[0][itow][i]
    //	     << " " <<  phi_bias_m7[0][itow][i]<< " " <<  phi_bias_m8[0][itow][i]<< " " <<  phi_bias_m9[0][itow][i]<< " " <<  phi_bias_m10[0][itow][i]
    //	     << endl;
    //  }
    //
    //}
  ...
  //return;
    ...
    //for (int itow=0; itow<16; ++itow) {
      for (int itow=2; itow<3; ++itow) {
      g_phi_bias_miss[0][itow] = new TGraphErrors(7,pt_bin,phi_bias_m5[0][itow],pt_bin_width,phi_dummy[0][itow]);
    ...
    //for (int itow=0; itow<16; ++itow) {
      for (int itow=2; itow<3; ++itow) {
      g_phi_bias_miss[1][itow] = new TGraphErrors(7,pt_bin,phi_bias_m6[0][itow],pt_bin_width,phi_dummy[0][itow]);
    ...
    //for (int itow=0; itow<16; ++itow) {
      for (int itow=2; itow<3; ++itow) {
      g_phi_bias_miss[2][itow] = new TGraphErrors(7,pt_bin,phi_bias_m7[0][itow],pt_bin_width,phi_dummy[0][itow]);
    ...
    //for (int itow=0; itow<16; ++itow) {
      for (int itow=2; itow<3; ++itow) {
      g_phi_bias_miss[3][itow] = new TGraphErrors(7,pt_bin,phi_bias_m8[0][itow],pt_bin_width,phi_dummy[0][itow]);
    ...
    //for (int itow=0; itow<16; ++itow) {
      for (int itow=2; itow<3; ++itow) {
      g_phi_bias_miss[4][itow] = new TGraphErrors(7,pt_bin,phi_bias_m9[0][itow],pt_bin_width,phi_dummy[0][itow]);
    ...
    //for (int itow=0; itow<16; ++itow) {
      for (int itow=2; itow<3; ++itow) {
      g_phi_bias_miss[5][itow] = new TGraphErrors(7,pt_bin,phi_bias_m10[0][itow],pt_bin_width,phi_dummy[0][itow]);
    ...
    leg[18] = new TLegend(0.449749,0.438811,0.659548,0.858392);
	...
	g_phi_bias_miss[i][itow]->SetMinimum(-5.e-6);
	g_phi_bias_miss[i][itow]->SetMaximum(25.e-6);
	g_phi_bias_miss[i][itow]->GetXaxis()->SetTitle("p_{T}^{bin} [GeV/c]");
	g_phi_bias_miss[i][itow]->GetYaxis()->SetTitle("#LT#phi_{gen} - #phi_{fit}#GT [rad]");
    ...
    leg[18]->Draw();
    canvas[18]->Update();
    canvas[18]->SaveAs("phi_bias_miss.png");
  ...
  //====================================================================================================================
    ...
    //for (int itow=0; itow<16; ++itow) {
      for (int itow=2; itow<3; ++itow) {
      g_phi_sigma_miss[0][itow] = new TGraphErrors(7,pt_bin,phi_sigma_m5[0][itow],pt_bin_width,phi_dummy[0][itow]);
    ...
    //for (int itow=0; itow<16; ++itow) {
      for (int itow=2; itow<3; ++itow) {
      g_phi_sigma_miss[1][itow] = new TGraphErrors(7,pt_bin,phi_sigma_m6[0][itow],pt_bin_width,phi_dummy[0][itow]);
    ...
    //for (int itow=0; itow<16; ++itow) {
      for (int itow=2; itow<3; ++itow) {
      g_phi_sigma_miss[2][itow] = new TGraphErrors(7,pt_bin,phi_sigma_m7[0][itow],pt_bin_width,phi_dummy[0][itow]);
    ...
    //for (int itow=0; itow<16; ++itow) {
      for (int itow=2; itow<3; ++itow) {
      g_phi_sigma_miss[3][itow] = new TGraphErrors(7,pt_bin,phi_sigma_m8[0][itow],pt_bin_width,phi_dummy[0][itow]);
    ...
    //for (int itow=0; itow<16; ++itow) {
      for (int itow=2; itow<3; ++itow) {
      g_phi_sigma_miss[4][itow] = new TGraphErrors(7,pt_bin,phi_sigma_m9[0][itow],pt_bin_width,phi_dummy[0][itow]);
    ...
    //for (int itow=0; itow<16; ++itow) {
      for (int itow=2; itow<3; ++itow) {
      g_phi_sigma_miss[5][itow] = new TGraphErrors(7,pt_bin,phi_sigma_m10[0][itow],pt_bin_width,phi_dummy[0][itow]);
    ...
    leg[19] = new TLegend(0.449749,0.438811,0.659548,0.858392);
	...
	//g_phi_sigma_miss[i][itow]->SetMinimum(-5.e-6);
	//g_phi_sigma_miss[i][itow]->SetMaximum(25.e-6);
	g_phi_sigma_miss[i][itow]->GetXaxis()->SetTitle("p_{T}^{bin} [GeV/c]");
	g_phi_sigma_miss[i][itow]->GetYaxis()->SetTitle("#sigma_{#phi_{gen} - #phi_{fit}} [rad]");
    ...
    leg[19]->Draw();
    canvas[19]->Update();
    canvas[19]->SaveAs("phi_sigma_miss.png");
